...
namespace AppWindow
{
	public class DemoApplicationWindow : Window
	{
		...
		void AddActions ()
		{
			...
			//Builder uim = new Builder();
			//uim.AddFromString(uiInfo);
			//uim.Autoconnect(group);
		...
		private void ActionActivated (object sender, EventArgs a)
		{
			...
			md = new MessageDialog (this, DialogFlags.DestroyWithParent, MessageType.Info, ButtonsType.Close,
						"You activated action: \"{0}\" of type \"{1}\"", null, action.GetType ());
			md.Run ();
			md.Dispose();
		}
		...
		private void RadioActionActivated (object sender, ChangedArgs args)
		{
			...
			if (action.Active) {
				md = new MessageDialog (this, DialogFlags.DestroyWithParent, MessageType.Info, ButtonsType.Close,
							...
							null, action.GetType (), args.Current.Value);
				md.Run ();
				md.Dispose();
			}
		}
		...
		protected override bool OnWindowStateEvent (Gdk.EventWindowState evt)
		{
			if ((evt.ChangedMask & (Gdk.WindowState.Maximized | Gdk.WindowState.Fullscreen)) != 0)
				this.HasDefault = (evt.NewWindowState & (Gdk.WindowState.Maximized | Gdk.WindowState.Fullscreen)) == 0;
			return false;
		}
